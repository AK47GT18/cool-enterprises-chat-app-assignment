...
import { Mail, Lock, ArrowRight, Loader2, CircleDashed, Eye, EyeOff, CheckCircle, XCircle, AlertCircle } from "lucide-react";
...
function LoginForm() {
  ...
  const [error, setError] = useState(reason === 'unauthorized' ? "Your session has expired or you've logged in from another device. Please log in again." : "");
  const [validationErrors, setValidationErrors] = useState<Record<string, string>>({});
  const [touched, setTouched] = useState<Record<string, boolean>>({});
  const [showPassword, setShowPassword] = useState(false);

  // ... (rest of the component logic moved here)
...
export default function LoginPage() {
  return (
    <div className="min-h-screen w-full flex items-center justify-center p-4 relative overflow-hidden bg-white">
      ...
      >
        <Suspense fallback={<div className="text-center p-10 bg-white rounded-2xl border">Loading access...</div>}>
          <LoginForm />
        </Suspense>
      </motion.div>
    </div>
